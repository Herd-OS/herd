...
func (c *ClaudeAgent) SynthesizeReviewNonConvergence(ctx context.Context, input agent.ReviewSynthesisInput, opts agent.ReviewSynthesisOptions) (*agent.ReviewSynthesisResult, error) {
	...
	args := []string{"--dangerously-skip-permissions", "--system-prompt", prompt.ReviewSynthesisSystemPrompt}
	if c.Model != "" {
		args = append(args, "--model", c.Model)
	}
	args = append(args, "-p")
	...
	runOnce := func() (string, string, error) {
		...
		if err := process.Run(ctx, process.Command{
			...
			Args:         args,
			Dir:          opts.RepoRoot,
			Stdin:        strings.NewReader(synthesisPrompt),
			Stdout:       io.MultiWriter(os.Stdout, &stdout),
			Stderr:       io.MultiWriter(os.Stderr, &stderr),
			ProcessGroup: true,
		}); err != nil {
