...
- **Batch PR merge conflict** — If an already-open batch PR becomes conflicted with its base branch, post `@herd-os resolve-conflicts` on the batch PR. HerdOS checks live GitHub mergeability, waits briefly when mergeability is unknown, and creates a focused conflict-resolution issue only when the PR currently conflicts with base. Use generic `@herd-os fix` for normal code fixes; it still includes PR conversation history for worker context.
...
- **Review non-convergence** — If several completed review-fix cycles do not reduce repeated package/root-cause clusters, HerdOS can optionally synthesize the dominant architectural/root-cause pattern, then creates and dispatches one strategy-level fix issue instead of asking for another `@herd-os fix`.
