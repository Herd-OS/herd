...
You can interact with HerdOS by posting `/herd` slash-comment commands as comments
...
Quotes around the prompt text are optional. You can paste error logs, JSON snippets, or any text directly after the command — everything after the command name (including subsequent lines) is treated as the prompt. The quoted format (`/herd fix "description"`) is also supported.
