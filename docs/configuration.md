...
The existing **high-volume deterministic route** applies when the latest actionable findings, after deduplication, meet an internal floor of eight. HerdOS requires the configured completed-cycle minimum and uses finding-count trends, concrete repeated package/subsystem clusters, and supporting repeated root-cause evidence to decide whether the loop is no longer converging. After deterministic eligibility succeeds, agent synthesis is optional and may refine the strategy. If synthesis is disabled, unavailable, invalid, low confidence, or rejected by safety gates, HerdOS uses the unchanged deterministic strategy fallback. Issues from this route are titled `Review strategy fix (cycle N): <root cause>`.

The **low-volume architectural oscillation route** applies only when the latest review has two through seven actionable deduplicated findings. Synthesis must be enabled, and HerdOS requires `max(3, min_completed_cycles)` completed review-fix cycles within `window`. Evidence must form a chronological `review → completed fix → next review` chain with distinct non-empty head SHAs. A conservative deterministic gate only establishes that real actionable findings recur in one normalized subsystem; it does not treat token overlap or package location as proof of one behavior. Current-batch requirements and eligible findings receive stable evidence IDs, and synthesis must cite them. After strict provenance validation, a separate bounded no-tools agent pass must approve with high confidence that the findings share one architectural root cause, the strategy addresses it, and any reinterpretation preserves the cited safety property. Only then is `Review strategy fix: <architectural root cause>` created.
