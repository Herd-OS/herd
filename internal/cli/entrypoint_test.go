...
func TestEntrypoint_ControlPlaneRunnerRegistration(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "request nonce generated by helper",
			want: `HERD_RUNNER_REQUEST_NONCE="${HERD_RUNNER_REQUEST_NONCE:-$(generate_runner_request_nonce)}"`,
		},
	}
...
func TestEntrypoint_DefaultRunnerRequestNonceUsesEntropy(t *testing.T) {
	script := readEntrypoint(t)

	assert.Contains(t, script, "generate_runner_request_nonce()")
	assert.Contains(t, script, "/proc/sys/kernel/random/uuid")
	assert.Contains(t, script, "uuidgen")
	assert.Contains(t, script, "/dev/urandom")
	assert.NotContains(t, script, `HERD_RUNNER_REQUEST_NONCE="${HERD_RUNNER_REQUEST_NONCE:-$(date +%s)-${HOSTNAME:-runner}-$$}"`)
}
