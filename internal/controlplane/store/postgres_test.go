...
func TestPostgresStoreMethods(t *testing.T) {
	...
	require.NoError(t, store.FailIdempotencyKey(ctx, "key-1", "late failure"))
	gotKey, err = store.GetIdempotencyKey(ctx, "key-1")
	require.NoError(t, err)
	assert.Equal(t, "completed", gotKey.Status)
	assert.Equal(t, "result-1", gotKey.ResultRef)
	created, err = store.AcquireIdempotencyKey(ctx, IdempotencyKey{Key: "key-2", Scope: "commands", Status: "started"})
	require.NoError(t, err)
	assert.True(t, created)
	require.NoError(t, store.FailIdempotencyKey(ctx, "key-2", "boom"))
	gotKey, err = store.GetIdempotencyKey(ctx, "key-2")
	require.NoError(t, err)
	assert.Equal(t, "failed", gotKey.Status)
	assert.Equal(t, "boom", gotKey.ResultRef)
	created, err = store.AcquireIdempotencyKey(ctx, IdempotencyKey{Key: "key-3", Scope: "commands", Status: "started"})
	require.NoError(t, err)
	assert.True(t, created)
	require.NoError(t, store.FailIdempotencyKey(ctx, "key-3", "failed_pre_call:client setup"))
	gotKey, err = store.GetIdempotencyKey(ctx, "key-3")
	require.NoError(t, err)
	assert.Equal(t, "failed_pre_call", gotKey.Status)
	assert.Equal(t, "client setup", gotKey.ResultRef)
	created, err = store.AcquireIdempotencyKey(ctx, IdempotencyKey{Key: "key-4", Scope: "commands", Status: "started"})
	require.NoError(t, err)
	assert.True(t, created)
	require.NoError(t, store.FailIdempotencyKey(ctx, "key-4", "repair_required:github timeout"))
	gotKey, err = store.GetIdempotencyKey(ctx, "key-4")
	require.NoError(t, err)
	assert.Equal(t, "repair_required", gotKey.Status)
...
func TestMemoryStore(t *testing.T) {
	...
	require.NoError(t, s.FailIdempotencyKey(ctx, "k1", "late failure"))
	gotKey, err = s.GetIdempotencyKey(ctx, "k1")
	require.NoError(t, err)
	assert.Equal(t, "completed", gotKey.Status)
	assert.Equal(t, "ref", gotKey.ResultRef)
	created, err = s.AcquireIdempotencyKey(ctx, IdempotencyKey{Key: "k2"})
	require.NoError(t, err)
	assert.True(t, created)
	require.NoError(t, s.FailIdempotencyKey(ctx, "k2", "boom"))
	gotKey, err = s.GetIdempotencyKey(ctx, "k2")
	require.NoError(t, err)
	assert.Equal(t, "failed", gotKey.Status)
	assert.Equal(t, "boom", gotKey.ResultRef)
	created, err = s.AcquireIdempotencyKey(ctx, IdempotencyKey{Key: "k3"})
	require.NoError(t, err)
	assert.True(t, created)
	require.NoError(t, s.FailIdempotencyKey(ctx, "k3", "failed_pre_call:client setup"))
	gotKey, err = s.GetIdempotencyKey(ctx, "k3")
	require.NoError(t, err)
	assert.Equal(t, "failed_pre_call", gotKey.Status)
	assert.Equal(t, "client setup", gotKey.ResultRef)
	created, err = s.AcquireIdempotencyKey(ctx, IdempotencyKey{Key: "k4"})
	require.NoError(t, err)
	assert.True(t, created)
	require.NoError(t, s.FailIdempotencyKey(ctx, "k4", "repair_required:github timeout"))
	gotKey, err = s.GetIdempotencyKey(ctx, "k4")
	require.NoError(t, err)
	assert.Equal(t, "repair_required", gotKey.Status)
	assert.Equal(t, "github timeout", gotKey.ResultRef)
	...
	require.NoError(t, err)
	assert.False(t, created)
	expiredLockAt := time.Now().UTC().Add(-time.Hour)
	created, err = s.AcquireReviewLock(ctx, ReviewLock{RepositoryID: 2, PRNumber: 4, HeadSHA: "sha", Holder: "old", ExpiresAt: expiredLockAt, AcquiredAt: expiredLockAt.Add(-time.Hour)})
	require.NoError(t, err)
	assert.True(t, created)
