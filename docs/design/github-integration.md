...
**Concurrent consolidation:** A single `herd integrator worker-cycle --run-id <N>` invocation holds one durable batch lock across consolidation, advancement, review, and CI checking. Its consolidation phase is milestone-wide: it resolves the worker issue and milestone from the workflow run inputs, enumerates every `herd/status:done` issue in that milestone whose worker branch is still on the remote, and merges each in a deterministic, idempotent loop. Worker `workflow_run.head_branch` can be `main` for `workflow_dispatch` workers, so it is diagnostic data rather than the batch selector. Same-batch Integrator mutations are serialized by the durable Herd batch lock; different batch numbers do not share that lock. If a same-batch worker completion loses the lock, Herd marks the issue `herd/integrator-pending`; the active worker cycle performs a fresh broad consolidation scan before PR review or CI eligibility. If a run is cancelled mid-loop and leaves stranded worker branches, the next successful integrator run re-scans the milestone and picks them up automatically. If two pushes into the batch branch still race (e.g., a manual `/herd integrate` overlapping with a `workflow_run` trigger), the loser observes a non-fast-forward push, relabels the affected issue as `herd/status:failed` for retry, and continues with the next candidate.
...
|-----|-----------|-------------------|----------|
| `integrate` | `herd-integrate-worker-{workflow_run.id}` | false | Lets separate worker-completion events run without collapsing behind `main`; same-batch safety is enforced by the durable batch lock after Herd resolves the worker issue and milestone. |
| `check-ci-workflow-completion` | `herd-integrate-ci-{head_branch}` | false | Optional when `integrator.ci_workflows` is configured. Serializes configured CI workflow completion handling by batch head branch. |
...
**Cross-job limitation:** Workflow concurrency cannot express one shared batch or review group across every trigger HerdOS supports. Different event payloads expose different identifiers (`workflow_run`, `check_run`, `issues`, `pull_request_review`, and `issue_comment`), and manual commands do not always have the same batch PR fields available at expression time. HerdOS therefore treats workflow concurrency as a secondary guard. Same-batch consolidation, advancement, CI handling, and review entry are guarded by the batch lock; the narrower correctness guarantee for one active agent review on a PR head comes from the application-level GitHub-backed review lock.
