...
jobs:
  publish:
    if: vars.HERD_ENABLED == 'true'
    runs-on: {{ if eq (len .ImagePublishRunsOn) 1 }}{{ index .ImagePublishRunsOn 0 }}{{ else }}[{{ range $i, $r := .ImagePublishRunsOn }}{{ if $i }}, {{ end }}{{ $r }}{{ end }}]{{ end }}
```
...
- Multi-label (any list whose `len > 1` — for example `["self-hosted", "<any-label>"]` or `["self-hosted", "linux", "x64", "<any-label>"]`) renders as the YAML flow list `runs-on: [self-hosted, <any-label>]`, preserving order and emitting elements verbatim with `, ` separators.

The template's `RenderWorkflow` data struct (see existing template
plumbing for the worker/integrator/monitor workflows) needs an
`ImagePublishRunsOn []string` field added; populate from
`cfg.ImagePublish.RunsOn` at render time.
...
- One-paragraph reminder that the publish workflow only runs on
  `workflow_dispatch` (per #713), so build cadence is operator-driven
  — sizing the publisher host for occasional bursts rather than
  continuous load is correct.
...
- `publish_runner_workflow_test.go` (existing): extend
  ...
  - **regression guard**: assert the existing `workflow_dispatch:` trigger, `packages: write` permission, and `if: vars.HERD_ENABLED == 'true'` gate are all still present (the existing assertions cover most of this — keep them).
