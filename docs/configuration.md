...
HerdOS watches recent review result comments and completed review-fix issues before creating another review-fix issue. It uses two internal routes. Both operate within `window`, and both require a concrete recurring package or subsystem rather than broad directory coincidence, generic terminology, or synthetic review metadata. The feature is enabled by default.

The existing **high-volume deterministic route** applies when the latest actionable findings, after deduplication, meet an internal floor of eight. HerdOS requires the configured completed-cycle minimum and uses finding-count trends, concrete repeated package/subsystem clusters, and supporting repeated root-cause evidence to decide whether the loop is no longer converging. After deterministic eligibility succeeds, agent synthesis is optional and may refine the strategy. Ordinary synthesis does not add an unnecessary verifier call. If synthesis proposes a requirement reinterpretation, current-batch requirement provenance and independent semantic verification are mandatory before that synthesized strategy can be used. If synthesis is disabled, unavailable, invalid, low confidence, lacks valid reinterpretation provenance, or fails verification, HerdOS uses the unchanged deterministic strategy fallback. Issues from this route are titled `Review strategy fix (cycle N): <root cause>`.

The **low-volume architectural oscillation route** applies only when the latest review has two through seven actionable deduplicated findings. Synthesis must be enabled, and HerdOS requires `max(3, min_completed_cycles)` completed review-fix cycles within `window`. Evidence must form a chronological `review → completed fix → next review` chain with distinct non-empty head SHAs. A conservative deterministic gate only establishes that real actionable findings recur in one normalized subsystem; it does not treat token overlap or package location as proof of one behavior. Current-batch requirements and eligible findings receive stable evidence IDs. Synthesis must cite findings from at least three completed historical cycles plus a finding from the latest review; the latest review is never counted as a completed historical fix cycle. After strict provenance validation, a separate bounded no-tools agent pass receives all bounded eligible evidence plus the synthesis-cited ID set, so it can detect cherry-picking, contradictions, or an unrelated current review. It must approve with high confidence that the findings share one architectural root cause, the strategy addresses it, and any reinterpretation preserves the cited safety property. Only then is `Review strategy fix: <architectural root cause>` created.

When low-volume synthesis is disabled, either synthesis or verification errors or times out, output is malformed or nil, provenance is invalid, confidence is low, or either agent declines/rejects escalation, HerdOS returns to the ordinary endpoint review-fix flow. The low-volume route never uses the deterministic strategy fallback. An active ready or in-progress ordinary review fix or strategy fix pauses review and non-convergence evaluation, so HerdOS does not run duplicate synthesis or post duplicate review-result comments while that work is outstanding.

When either route escalates, HerdOS creates the strategy-level fix issue internally and dispatches its worker directly. Active strategy fixes suppress duplicates by title and fingerprint. Completed strategy fixes do not suppress recurrence forever: if the same fingerprint returns after a completed but incomplete attempt, HerdOS may create another strategy issue and records the prior attempt. When route eligibility fails, HerdOS posts or renders the normal HerdOS Agent Review result and creates ordinary review-fix issues through the existing review flow.
...
For the high-volume route only, synthesis or mandatory reinterpretation-verification failure falls back to deterministic strategy issue behavior without routine PR-comment noise. For the low-volume route, synthesis or independent verification failure returns to ordinary endpoint fixes. Prompt budgets and both timeouts are conservative internal implementation details, not public configuration.

The two- and eight-finding boundaries, the three-cycle evidence span, and repeated cluster and chronology requirements are conservative internal policy constants, not configurable knobs.
