...
func (h RegisterHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.store.UpsertRepository(r.Context(), store.Repository{
	...
	repo, err := h.store.GetRepository(r.Context(), req.Owner, req.Name)
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "load repository registration: storage unavailable"})
		return
	}
