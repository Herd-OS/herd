...
func (s *registerFakeStore) UpsertRepository(_ context.Context, r store.Repository) error {
	if s.err != nil {
		return s.err
	}
	if r.ID == 0 {
...
func (s *registerFakeStore) GetRepository(_ context.Context, owner string, name string) (store.Repository, error) {
	if s.err != nil {
		return store.Repository{}, s.err
	}
	repo, ok := s.repositories[owner+"/"+name]
	if !ok {
