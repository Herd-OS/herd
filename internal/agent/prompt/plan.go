...
// PlanningPromptTemplate is the text/template source rendered for an
...
// RenderPlanningPrompt renders [PlanningPromptTemplate] using the
// repository context derived from opts.
...
// ValidatePlan checks that the plan is well-formed: a non-empty batch name,
// at least one task, every task has a title and acceptance criteria, and all
// depends_on indices are in-range and non-self-referential.
