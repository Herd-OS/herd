...
5. **Create runner files** — `Dockerfile.herd_runner` (which `FROM`s a published `ghcr.io/herd-os/herd-runner-<flavor>` base image auto-selected from your manifests), `entrypoint.herd.sh`, `docker-compose.herd.yml`, and `.env.herd.example` for self-hosted runner setup
