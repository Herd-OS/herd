...
The **low-volume architectural oscillation route** applies only when the latest review has two through seven actionable deduplicated findings. Synthesis must be enabled, and HerdOS requires `max(3, min_completed_cycles)` completed review-fix cycles within `window`. Evidence must span at least three completed cycles in chronological `review → completed fix → next review` order, with a distinct reviewed head SHA at every step. A concrete subsystem must recur together with architectural or state-machine evidence, such as an alternating behavior or a symptom that moves between endpoints after successive fixes. Package matching alone, unrelated findings from the same package, broad directory matches, and generic, synthetic, or no-issue evidence are not sufficient. This route creates a strategy issue only when synthesis is accepted after confidence, coherence, alignment, safety, and fingerprint checks. Its title is `Review strategy fix: <architectural root cause>`.

When low-volume synthesis is disabled, errors, times out, returns invalid or unsafe output, has low confidence, is incoherent, or declines escalation, HerdOS returns to the ordinary endpoint review-fix flow. The low-volume route never uses the deterministic strategy fallback. An active ready or in-progress ordinary review fix or strategy fix pauses review and non-convergence evaluation, so HerdOS does not run duplicate synthesis or post duplicate review-result comments while that work is outstanding.
