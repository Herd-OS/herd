...
func TestRunHostedReviewReadOnlyStandalonePRRunsAgent(t *testing.T) {
	...
	assert.Equal(t, findings[0].Description, result.Findings[0].Description)
	assert.NotEmpty(t, result.Findings[1].Fingerprint)
	assert.Equal(t, "medium", result.Findings[1].Severity)
	assert.Equal(t, findings[1].Description, result.Findings[1].Description)
	require.Len(t, ag.reviewOpts, 1)
